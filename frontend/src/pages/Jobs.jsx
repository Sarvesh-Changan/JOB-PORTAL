...
const Jobs = () => {
  ...
  // Run fetchJobs only on initial load and when city or niche changes.
  // Notice that searchKeyword is intentionally omitted.
  useEffect(() => {
    if (error) {
      toast.error(error);
      dispatch(clearAllJobErrors());
    }
    // On initial load or filter change, fetch jobs without the search keyword.
    dispatch(fetchJobs(city, niche, ""));
  }, [dispatch, error, city, niche]);

  // This function triggers a search using the current searchKeyword.
  const handleSearch = () => {
            ...
            <div className="w-3/4 max-[730px]:w-full">
              ...
              <div className="grid grid-cols-[repeat(auto-fit,minmax(450px,1fr))] gap-[40px] py-[50px] w-full max-[730px]:flex max-[730px]:flex-col max-[730px]:p-[20px] max-[730px]:border-t max-[730px]:border-gray-500 max-[730px]:mt-[20px]">
                {jobs && jobs.length > 0 ? (
                  jobs.map((element) => (
                    <div
                      className="transition duration-300 bg-[#f5f5f5] px-[40px] py-[20px] flex flex-col gap-[5px] rounded-[6px] hover:bg-[rgba(133,133,124,0.19)]"
                      key={element._id}
                    >
